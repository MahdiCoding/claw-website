...
function renderProducts(filter = 'all') {
  ...
  grid.innerHTML = filtered.map(p => {
    ...
    const badge   = currentLang === 'ar' ? p.badgeAr : p.badge;
    const imgSrc  = p.image;
    const sizes   = p.sizes.map(s =>
      `<button class="size-btn" onclick="selectSize(this, ${p.id})">${s}</button>`
    ).join('');
        ...
        <div class="product-img-wrap">
          ...
          <div class="product-overlay">
            <button class="quick-add" onclick="quickAdd(${p.id})"
              data-en="Add to Bag" data-ar="أضف للحقيبة">
              ${currentLang === 'ar' ? 'أضف للحقيبة' : 'Add to Bag'}
            </button>
          </div>
        </div>
...
function submitOrder() {
  ...
  // In a real deployment, send this data to your backend / WhatsApp / email
  console.log('ORDER:', { name, phone, address, cart });
